...
export namespace ServerProxy {
  export function proxy(url: string | URL, req: { raw: { headers: HeadersInit }; method: string; body: any; signal: AbortSignal }) {
    return fetch(
      new Request(url, {
        method: req.method,
        headers: headers(new Request("http://localhost", { headers: req.raw.headers })),
