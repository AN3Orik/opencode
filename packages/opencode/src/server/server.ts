  ...
  export function createApp(opts: { cors?: string[] }) {
    ...
    return app.all("/*", async (c) => {
      ...
      try {
        ...
        const normalizedPath = process.platform === "win32" && distPath.startsWith("/") ? distPath.slice(1) : distPath
      ...
      const response = await ServerProxy.proxy(`https://app.opencode.ai${path}`, {
        method: c.req.method,
        body: c.req.raw.body,
